...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--review-surface",
        type=pathlib.Path,
        help="Optional review_surface.v1 JSON generated by `forager ondesk review-surface --json`.",
    )
...
def review_surface_count(value: dict[str, Any], *path: str) -> int:
    current: Any = value
    for key in path:
        if not isinstance(current, dict):
            return 0
        current = current.get(key)
    return int(current) if isinstance(current, int) and current >= 0 else 0


def review_surface_string_list(value: Any, *, limit: int = 4) -> list[str]:
    if not isinstance(value, list):
        return []
    return [str(item).strip() for item in value[:limit] if str(item).strip()]


def review_surface_artifact_summaries(surface: dict[str, Any], *, limit: int = 4) -> list[dict[str, str]]:
    artifacts = surface.get("artifacts") if isinstance(surface.get("artifacts"), dict) else {}
    summaries = artifacts.get("summary") if isinstance(artifacts.get("summary"), list) else []
    ...
    for item in summaries[:limit]:
        ...
        retention = str(item.get("retention_class") or "").strip()
        if label or why:
            result.append({"label": label or "Artifact", "why_it_matters": why, "retention_class": retention})
    return result
...
def review_surface_projection(surface: dict[str, Any]) -> dict[str, Any]:
    ...
    accepted_truth = surface.get("accepted_truth") if isinstance(surface.get("accepted_truth"), dict) else {}
    ...
    return {
        ...
        "status": {
            ...
            "severity": str(status.get("severity") or "").strip(),
            "summary": str(status.get("summary") or "").strip(),
        },
        "accepted_truth": {
            "status": str(accepted_truth.get("status") or "").strip(),
        ...
        "closeout": {
            ...
            "review_status": str(closeout.get("review_status") or "").strip(),
            "unresolved_risks": review_surface_string_list(closeout.get("unresolved_risks"), limit=5),
        },
        "runtime": {
            "progress_summary": str(runtime.get("progress_summary") or "").strip(),
        },
        "decisions": {
            "open_count": review_surface_count(surface, "decisions", "open_count"),
        },
        "adaptive_wiki": {
            "candidate_count": review_surface_count(surface, "adaptive_wiki", "candidate_count"),
            "review_due_count": review_surface_count(surface, "adaptive_wiki", "review_due_count"),
        },
...
def review_surface_evidence_lines(projection: dict[str, Any]) -> list[str]:
    ...
    accepted_truth = (
        projection.get("accepted_truth") if isinstance(projection.get("accepted_truth"), dict) else {}
    )
    closeout = projection.get("closeout") if isinstance(projection.get("closeout"), dict) else {}
    decisions = projection.get("decisions") if isinstance(projection.get("decisions"), dict) else {}
    adaptive_wiki = (
    ...
    if risks:
        lines.append("Unresolved risks: " + "; ".join(str(item) for item in risks[:3]))
    ...
    if open_count or wiki_candidates:
        lines.append(f"Review queue: open decisions {open_count}건, wiki candidates {wiki_candidates}건.")
    for artifact in projection.get("artifact_summaries", [])[:3]:
        if not isinstance(artifact, dict):
            continue
        label = str(artifact.get("label") or "Artifact").strip()
        why = str(artifact.get("why_it_matters") or "").strip()
        if why:
...
def build_request(args: argparse.Namespace) -> dict[str, Any]:
    ...
    review_surface = load_json_object(args.review_surface)
    review_surface_summary = review_surface_projection(review_surface)
    ...
    evidence = [
        ...
        *review_surface_evidence_lines(review_surface_summary),
    ...
    if review_surface_summary:
        approval_brief["review_surface"] = review_surface_summary
    trace_refs = [
        ref
        for ref in (
            ...
            decision_records.trace_ref("artifact", "review_surface", args.review_surface),
        )
        if ref is not None
    ]
    ...
    request: dict[str, Any] = {
        ...
        "artifacts": {
            ...
            "review_surface": str(args.review_surface) if args.review_surface else None,
        },
        "summary": {
            "project_key": args.project_key,
