...
def _offdesk_judge_prompt(entry: Dict[str, Any], task: Dict[str, Any], team_dir: Path) -> str:
    ...
    payload = {
        ...
        "job_contract": str(task.get("job_contract_summary", "")).strip() or "-",
        "job_goal": str(task.get("job_contract_goal", "")).strip() or "-",
        "job_scope": list(task.get("job_contract_scope") or []),
        "debug_packet": str(task.get("debug_packet_summary", "")).strip() or "-",
        "debug_symptom": str(task.get("debug_packet_symptom", "")).strip() or "-",
        "debug_next_step": str(task.get("debug_packet_next_step", "")).strip() or "-",
        "phase_checkpoint": str(task.get("phase_checkpoint_summary", "")).strip() or "-",
        "phase_checkpoint_rows": list(task.get("phase_checkpoint_rows") or []),
        "followup_brief_status": str(task.get("followup_brief_status", "")).strip() or "-",
...
def handle_orch_task_command(
    *,
    ...
    if cmd in {"status", "orch-status"}:
        ...
        worker_items_line = ""
        worker_item_tokens_line = ""
        worker_item_classes_line = ""
        worker_item_class_tokens_line = ""
        worker_records_line = ""
        worker_record_tokens_line = ""
        worker_record_rows_line = ""
        worker_record_row_tokens_line = ""
        worker_record_set_line = ""
        worker_record_set_tokens_line = ""
        worker_preflight_line = ""
        worker_preflight_rows_line = ""
        ...
        try:
            ...
            if str(team_dir):
                ...
                latest_worker_items_summary = str(
                    (latest_task or {}).get("background_run_worker_items_summary", "")
                ).strip()
                if (
                    latest_worker_items_summary in {"", "-"}
                    and latest_worker_module not in {"", "-", "general"}
                    and latest_worker_checklist_summary not in {"", "-"}
                ):
                    latest_worker_items_summary = str(
                        worker_task_contract.derive_worker_task_module_items(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                            ...
                            checklist={
                                "state": (latest_task or {}).get("background_run_worker_checklist_status"),
                                "summary_line": latest_worker_checklist_summary,
                            },
                        ).get("summary_line", "")
                    ).strip()
                if latest_worker_items_summary not in {"", "-"}:
                    worker_items_line = f"worker_items: {latest_worker_items_summary[:240]}\n"
                latest_worker_item_tokens = [
                    str(item).strip()
                    for item in (
                        (((latest_task or {}).get("background_run_worker_items")) if isinstance((latest_task or {}).get("background_run_worker_items"), list) else [])
                        or []
                    )
                    if str(item).strip()
                ]
                if latest_worker_item_tokens:
                    worker_item_tokens_line = (
                        "worker_item_tokens: " + ", ".join(latest_worker_item_tokens[:6])[:240] + "\n"
                    )
                latest_worker_item_classes_summary = str(
                    (latest_task or {}).get("background_run_worker_item_classes_summary", "")
                ).strip()
                if latest_worker_item_classes_summary in {"", "-"} and latest_worker_module not in {"", "-", "general"}:
                    latest_worker_item_classes_summary = str(
                        worker_task_contract.derive_worker_task_module_item_classes(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                            ...
                            items={
                                "module_kind": latest_worker_module,
                                "items": latest_worker_item_tokens,
                                "summary_line": latest_worker_items_summary,
                            },
                        ).get("summary_line", "")
                    ).strip()
                if latest_worker_item_classes_summary not in {"", "-"}:
                    worker_item_classes_line = (
                        f"worker_item_classes: {latest_worker_item_classes_summary[:240]}\n"
                    )
                latest_worker_item_class_tokens = [
                    str(item).strip()
                    for item in (
                        (((latest_task or {}).get("background_run_worker_item_classes")) if isinstance((latest_task or {}).get("background_run_worker_item_classes"), list) else [])
                        or []
                    )
                    if str(item).strip()
                ]
                if latest_worker_item_class_tokens:
                    worker_item_class_tokens_line = (
                        "worker_item_class_tokens: " + ", ".join(latest_worker_item_class_tokens[:6])[:240] + "\n"
                    )
                latest_worker_records_summary = str(
                    (latest_task or {}).get("background_run_worker_records_summary", "")
                ).strip()
                if latest_worker_records_summary in {"", "-"} and latest_worker_module not in {"", "-", "general"}:
                    latest_worker_records_summary = str(
                        worker_task_contract.derive_worker_task_module_records(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                            ...
                            item_classes={
                                "module_kind": latest_worker_module,
                                "classes": latest_worker_item_class_tokens,
                                "summary_line": latest_worker_item_classes_summary,
                            },
                        ).get("summary_line", "")
                    ).strip()
                if latest_worker_records_summary not in {"", "-"}:
                    worker_records_line = f"worker_records: {latest_worker_records_summary[:240]}\n"
                latest_worker_record_tokens = [
                    str(item).strip()
                    for item in (
                        (((latest_task or {}).get("background_run_worker_records")) if isinstance((latest_task or {}).get("background_run_worker_records"), list) else [])
                        or []
                    )
                    if str(item).strip()
                ]
                if latest_worker_record_tokens:
                    worker_record_tokens_line = (
                        "worker_record_tokens: " + ", ".join(latest_worker_record_tokens[:6])[:240] + "\n"
                    )
                latest_worker_record_rows_summary = str(
                    (latest_task or {}).get("background_run_worker_record_rows_summary", "")
                ).strip()
                if latest_worker_record_rows_summary in {"", "-"} and latest_worker_module not in {"", "-", "general"}:
                    latest_worker_record_rows_summary = str(
                        worker_task_contract.derive_worker_task_module_record_rows(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                            ...
                            records={
                                "module_kind": latest_worker_module,
                                "records": latest_worker_record_tokens,
                                "summary_line": latest_worker_records_summary,
                            },
                        ).get("summary_line", "")
                    ).strip()
                if latest_worker_record_rows_summary not in {"", "-"}:
                    worker_record_rows_line = f"worker_record_rows: {latest_worker_record_rows_summary[:240]}\n"
                latest_worker_record_row_tokens = [
                    str(item).strip()
                    for item in (
                        (((latest_task or {}).get("background_run_worker_record_rows")) if isinstance((latest_task or {}).get("background_run_worker_record_rows"), list) else [])
                        or []
                    )
                    if str(item).strip()
                ]
                if latest_worker_record_row_tokens:
                    worker_record_row_tokens_line = (
                        "worker_record_row_tokens: " + ", ".join(latest_worker_record_row_tokens[:6])[:240] + "\n"
                    )
                latest_worker_record_set_summary = str(
                    (latest_task or {}).get("background_run_worker_record_set_summary", "")
                ).strip()
                if latest_worker_record_set_summary in {"", "-"} and latest_worker_module not in {"", "-", "general"}:
                    latest_worker_record_set_summary = str(
                        worker_task_contract.derive_worker_task_module_record_set(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                            ...
                            record_rows={
                                "module_kind": latest_worker_module,
                                "rows": latest_worker_record_row_tokens,
                                "summary_line": latest_worker_record_rows_summary,
                            },
                        ).get("summary_line", "")
                    ).strip()
                if latest_worker_record_set_summary not in {"", "-"}:
                    worker_record_set_line = f"worker_record_set: {latest_worker_record_set_summary[:240]}\n"
                latest_worker_record_set_tokens = []
                for item in (
                    (((latest_task or {}).get("background_run_worker_record_set")) if isinstance((latest_task or {}).get("background_run_worker_record_set"), list) else [])
                    or []
                ):
                    ...
                    if note and note != "-":
                        token += f"|note={note}"
                    latest_worker_record_set_tokens.append(token[:160])
                if latest_worker_record_set_tokens:
                    worker_record_set_tokens_line = (
                        "worker_record_set_tokens: " + ", ".join(latest_worker_record_set_tokens[:6])[:240] + "\n"
                    )
                latest_worker_preflight_summary = str(
                    (latest_task or {}).get("background_run_worker_preflight_summary", "")
                ).strip()
                if latest_worker_preflight_summary in {"", "-"} and latest_worker_module not in {"", "-", "general"}:
                    latest_worker_preflight_summary = str(
                        worker_task_contract.derive_worker_task_module_preflight(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                ...
                if latest_worker_preflight_summary not in {"", "-"}:
                    worker_preflight_line = f"worker_preflight: {latest_worker_preflight_summary[:240]}\n"
                latest_worker_preflight_rows_summary = str(
                    (latest_task or {}).get("background_run_worker_preflight_rows_summary", "")
                ).strip()
                if latest_worker_preflight_rows_summary in {"", "-"} and latest_worker_module not in {"", "-", "general"}:
                    latest_worker_preflight_rows_summary = str(
                        worker_task_contract.derive_worker_task_module_preflight_rows(
                            {
                                "module_kind": latest_worker_module,
                                "module_policy": (latest_task or {}).get("background_run_task_contract_policy"),
                                "artifact_targets": (latest_task or {}).get("background_run_worker_update_stub_targets"),
                            ...
                            preflight={
                                "module_kind": latest_worker_module,
                                "state": (latest_task or {}).get("background_run_worker_preflight_status"),
                                "summary_line": latest_worker_preflight_summary,
                            },
                        ).get("summary_line", "")
                    ).strip()
                if latest_worker_preflight_rows_summary not in {"", "-"}:
                    worker_preflight_rows_line = (
                        f"worker_preflight_rows: {latest_worker_preflight_rows_summary[:240]}\n"
                    )
        ...
        send(
            ...
            f"{runner_line}{runner_note_line}{run_lock_line}{run_lock_note_line}{workspace_line}{document_registry_line}{model_routing_line}{model_registry_line}{judge_binding_line}{judge_probe_line}{judge_bridge_line}{replan_auto_routing_policy_line}{replan_auto_route_status_line}{manual_step_line}{canonical_writeback_line}{canonical_mutation_line}{worker_module_line}{worker_policy_line}{worker_gate_line}{worker_profile_line}{worker_checklist_line}{worker_items_line}{worker_item_tokens_line}{worker_item_classes_line}{worker_item_class_tokens_line}{worker_records_line}{worker_record_tokens_line}{worker_record_rows_line}{worker_record_row_tokens_line}{worker_record_set_line}{worker_record_set_tokens_line}{worker_preflight_line}{worker_preflight_rows_line}{worker_apply_accept_line}{worker_syncback_line}{escalation_binding_line}{escalation_probe_line}{slots_line}{queue_line}{scheduler_line}{worker_line}{external_line}{external_next_line}\n{status}",
            context="status",
            with_menu=False,
            reply_markup=_orch_status_reply_markup(manager_state, key, entry),
        )
