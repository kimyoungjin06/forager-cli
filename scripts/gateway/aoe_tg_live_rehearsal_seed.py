    ...
    (project_root / "tests" / "session.test.js").write_text(
        "\n".join(
            [
                ...
                "  const tokenStore = { clear: () => calls.push('clear') };",
                "",
                "  assert.deepEqual(handleLoginFailure({ code: 'session_expired' }, tokenStore), {",
                "    status: 'logged_out',",
                "    reason: 'session_expired',",
                "  });",
                ...
                "});",
                "",
                "test('does not clear persisted token for other login failures', () => {",
                "  const calls = [];",
                "  const tokenStore = { clear: () => calls.push('clear') };",
                "",
                "  assert.deepEqual(handleLoginFailure({ code: 'invalid_credentials' }, tokenStore), {",
                "    status: 'failed',",
    ...
    (project_root / "docs" / "analysis" / "auth_scope_inventory.md").write_text(
        "\n".join(
            [
                ...
                "- regression_evidence: node --test tests/session.test.js passes session_expired token clear and non-session_expired no-clear coverage",
    ...
    (project_root / "docs" / "analysis" / "package_scope_matrix.md").write_text(
        "\n".join(
            [
                ...
                "| --- | --- | --- | --- |",
                "| A | auth/session only | low risk token cleanup release | approve if UI copy stays unchanged and customer-facing expired-session copy remains outside this change |",
                "| B | auth/session + dashboard/session-banner | broader customer-facing release | approve if expired-session banner copy or ownership changes this cycle |",
                "",
    ...
    (project_root / "docs" / "handoff" / "operator_handoff.md").write_text(
        "\n".join(
            [
                ...
                "- validation evidence: `node --test tests/session.test.js` passes; coverage includes tokenStore.clear on session_expired and no clear for non-session_expired login failures.",
                ...
                "- operator-owned copy decision: choose whether auth/session or dashboard/session-banner owns customer-facing expired-session copy.",
    ...
    (project_root / "docs" / "reviews" / "reviewer_note.md").write_text(
        "\n".join(
            [
                ...
                "## Findings",
                "",
                "- blocker: none found in the implementation lane evidence.",
                "- high: none found in the implementation lane evidence.",
                "- medium: none found; the regression test uses the Node built-in runner.",
                "- low: dashboard/session-banner ownership remains an unresolved package-scope decision, not an auth/session implementation defect.",
                "",
                "## Implementation Evidence Reviewed",
                "",
                "- `src/session.js`: `handleLoginFailure` clears `tokenStore` only when `error.code` is `session_expired`, then returns `logged_out` with the same reason.",
                "- `tests/session.test.js`: uses `node:test` and `node:assert/strict`; covers the required positive `session_expired` token-clear path and the non-`session_expired` no-clear negative path.",
                "- `docs/analysis/auth_scope_inventory.md`: records auth/session as complete, the test command evidence, and dashboard/session-banner copy/ownership as an open package boundary.",
                "- `work_result`: marks L1 implementation complete and keeps operator package-boundary decision as the manual remainder.",
                "- `docs/handoff/operator_handoff.md`: preserves the requirement that this must remain manual_followup until operator arbitration.",
                "",
                "## Regression Risks",
                "",
                "- The UI/customer-facing expired-session copy may move into dashboard/session-banner if the operator chooses the broader release scope.",
                "- Release packaging can drift if auth/session and dashboard/session-banner are bundled before the operator selects option A or B in the package scope matrix.",
                "- The current implementation assumes `tokenStore.clear()` is available when `session_expired` is handled; no broader token-store contract was changed in this slice.",
                "",
                "## Test Gaps",
                "",
                "- `node --test tests/session.test.js` is the required validation command.",
                "- Covered: `session_expired` returns `logged_out` and calls `tokenStore.clear()` once.",
                "- Covered: non-`session_expired` login failure returns `failed` and does not call `tokenStore.clear()`.",
                "- No dashboard/session-banner regression test should be required unless the operator includes that package in this release scope.",
                "",
                "## Verdict",
                "",
                "- reviewer outcome: implementation lane acceptable; the Node built-in test runner has positive and negative regression coverage.",
                "- release outcome: manual_followup remains required for package-boundary arbitration.",
            ]
        )
        + "\n",
        encoding="utf-8",
    ...
    (project_root / "work_result").write_text(
        "\n".join(
            [
                ...
                "changed_files=src/session.js,tests/session.test.js,docs/analysis/auth_scope_inventory.md,docs/handoff/operator_handoff.md,docs/analysis/package_scope_matrix.md,docs/reviews/reviewer_note.md,work_result",
                "validation=node --test tests/session.test.js pass: tests=2 pass=2 fail=0",
