...
def task_team_observatory_snapshot(
    ...
    *,
    ...
    age_sec = max(0, int((current - created_dt).total_seconds()))
    idle_sec = max(0, int((current - updated_dt).total_seconds()))
    freshness_scope = "task"

    lanes: List[Dict[str, Any]] = []
    for row in lane_rows:
        threshold = _stale_threshold_for_row(row)
        is_stale = _stale_candidate(row) and idle_sec >= threshold
        note = _lane_note(row)
        lane = dict(row)
        lane.update(
            {
                ...
                "last_event_kind": "task_updated",
                "age_sec": age_sec,
                "idle_sec": idle_sec,
                "age_text": _duration_text(age_sec),
                "idle_text": _duration_text(idle_sec),
