    ...
    *,
    ...
    if bool(result.get("ok")):
        ...
        if task_result:
            ...
            record_rows = worker_task_contract.derive_worker_task_module_record_rows(
                launch_spec.get("provider_task_contract_json"),
                task_result,
                update_stub=update_stub,
                gate=gate,
            ...
            if record_set:
                result["task_record_set_summary"] = _trim(record_set.get("summary_line"), 320) or "-"
                result["task_record_set"] = list(record_set.get("records") or [])
            ...
            preflight_rows = worker_task_contract.derive_worker_task_module_preflight_rows(
                launch_spec.get("provider_task_contract_json"),
                task_result,
                update_stub=update_stub,
                gate=gate,
            ...
            if preflight_rows:
                result["task_preflight_rows_summary"] = _trim(preflight_rows.get("summary_line"), 320) or "-"
                result["task_preflight_rows"] = list(preflight_rows.get("rows") or [])
            if update_stub:
