...
| `workstation_surface.v1` | FD-008 | Workstation-level dashboard packet for health, capacity, project portfolio, latest decision state, top attention, scoped provenance, latest action-envelope receipt verdicts, latest decision-action execution results, post-closeout runtime handoffs, and accepted-truth recovery candidates. | Web dashboard consumes this packet read-only; append-only decision ledgers are projected through the latest record per decision id so receipted decisions do not remain open through older entries. |
| `decision_inbox_surface.v1` | FD-004, FD-008 | Read-only action-center projection of open decision records with urgency, allowed actions, stale guards, evidence refs, and CLI fallback. | Web and mobile surfaces may preview choices, but mutation still requires a later action envelope or decision receipt. |
| `runtime_dispatch_surface.v1` | FD-005, FD-008, FD-015 | Post-closeout projection of receipted decision-action closeouts, latest runtime preflights, latest dispatch receipts, and the next CLI fallback command for each handoff stage. | This surface is not part of the decision inbox and does not reopen receipted decisions; it only shows whether the next safe continuation is preflight, queue dispatch, or scheduler-gated tick. |
| `accepted_truth_recovery_surface.v1` | FD-007, FD-008, FD-015 | Read-only projection of latest closeout receipts that are not accepted truth, including follow-up decisions, revision/blocked receipts, retired-incomplete receipts, fallback CLI commands, and receipt-backed recovery action previews. | This surface does not record accepted truth, resolve follow-ups, retire closeouts, move files, or promote wiki state; it only separates recovery work from the open decision inbox and from accepted output. |
| `accepted_truth_recovery_action_envelope.v1` | FD-007, FD-008, FD-015 | Preview-only boundary for a scoped accepted-truth recovery fallback, including closeout review target ref, observed hash, nonce, TTL, issued/expiry timestamps, idempotency key, forbidden effects, and expected receipt schema. | Web/mobile may validate a recovery fallback envelope, but the envelope itself must not execute `closeout-decision`, retire a closeout, move files, promote wiki state, or record accepted truth. |
| `accepted_truth_recovery_action_receipt.v1` | FD-007, FD-008, FD-015 | Receipt proving that an accepted-truth recovery action envelope matched the current closeout review surface or was rejected as stale/invalid. | This receipt only proves the fallback preview is current; it is not an accepted-truth receipt and does not authorize runtime or project-file mutation. |
| `action_envelope.v1` | FD-004, FD-008, FD-015 | Preview-only boundary for a scoped UI action, including target ref, observed hash, nonce, TTL, issued/expiry timestamps, idempotency key, forbidden effects, and expected receipt schema. | Surfaces may render the envelope before execution; no mutation may execute until a receipt-backed action processor validates the envelope. |
| `action_envelope_receipt.v1` | FD-004, FD-008, FD-015 | Receipt proving that an action envelope was validated against the current decision ledger or rejected as stale/invalid before any mutation-capable path. | Web/mobile actions show the latest receipt verdict and stale reason from `workstation_surface.v1` instead of silently continuing from old UI state. |
| `action_execution_preflight.v1` | FD-004, FD-008, FD-015 | Second-stage receipt proving that a previously validated action-envelope receipt is still current, latest, non-stale, and matched to the live decision ledger. | Mutation-capable executors must start from this preflight instead of from a preview envelope or old receipt alone. |
| `decision_action_execution.v1` | FD-004, FD-008, FD-015 | Receipt proving that a supported decision action was applied or blocked from a ready action preflight into the canonical decision ledger path. | Web/mobile surfaces show the latest applied/blocked result; applied executions may append a decision handoff record, but they still do not dispatch runtime work, edit project files, or update accepted truth. |
| `decision_action_closeout.v1` | FD-004, FD-008, FD-015 | Receipt proving that an applied Web/mobile decision action handoff was closed into a canonical `DecisionReceipt`. | It may append a `receipted` decision record, but it still does not dispatch runtime work, edit project files, retarget providers, promote wiki state, or update accepted truth. |
| `runtime_dispatch_preflight.v1` | FD-005, FD-008, FD-015 | Receipt proving that a receipted decision-action closeout is current before runtime work can be queued. | It is still non-mutating; dispatch must start from this preflight rather than from an old closeout alone. |
| `runtime_dispatch_receipt.v1` | FD-005, FD-008, FD-015 | Receipt proving that runtime work was queued as an Offdesk task from a ready runtime dispatch preflight. | It may write `offdesk_tasks.json`, but it does not launch a process; actual launch still goes through `forager offdesk tick` and the existing scheduler gate. |
...
- `forager ondesk workstation-surface --json`.
