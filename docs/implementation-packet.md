...
## Work-Slice Execution Receipt Draft

`work_slice_execution_receipt.v1` is the planned receipt that lets closeout
judge each packet work slice from slice-level evidence instead of inheriting the
packet-level status.
...
Storage should stay close to runtime evidence. A future implementation can write
one JSONL stream such as `work_slice_receipts.jsonl` beside `result.json`,
runtime logs, or the closeout artifact directory, and later project an indexed
summary into profile state.
...
4. Implement `work_slice_execution_receipt.v1` so work slices can report
   completed, deferred, missing, or drifted status from slice-level evidence
   instead of inheriting packet-level status. The initial design contract is
   captured above; Rust state, collection, closeout projection, and tests remain
   to be added.
