...
- Baseline GitHub Actions bridge:
  ...
  - sets a ticket-scoped run name:
    - `external-background-<runner_target>-<ticket_id>`
...
- Remaining sync boundary:
  ...
  - automated GitHub run wait + artifact retrieval path:
    - `scripts/gateway/aoe-external-sidecar-sync.py watch-github-artifact --team-dir <team_dir> --run-id <run-id> --ticket-id <ticket> --runner github_runner --poll`
  - operator-free local import scheduling path:
    ...
    - `/control` and `/control/health` report scheduled GitHub import backlog counts and latest failure reasons
