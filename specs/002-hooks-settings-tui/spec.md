...
**Current Forager note**: This draft predates the sandbox removal work. It
remains useful for hooks/settings history, but current Forager runs hooks on
the host project path. New sandbox session creation is deferred and is not an
active requirement for this spec.
...
preferred setup commands without requiring a per-repo `.forager/config.toml`.
...
`.forager/config.toml`. Verify the global hooks execute during creation and
launch.

**Acceptance Scenarios**:

1. **Given** a user has `on_create = ["echo hello"]` in their global
   config, **When** they create a session for a repo with
   no `.forager/config.toml`, **Then** the `echo hello` command runs locally
   in the project directory during session creation.
2. **Given** a user has `on_launch = ["echo launching"]` globally,
   **When** they launch any session, **Then** the command runs on every
   launch in the host project directory.
3. **Given** a user has global hooks and the repo also defines hooks in
   `.forager/config.toml`, **When** the session is created/launched,
...
global-then-profile pattern already used for worktree, session, sound,
and other config sections.
...
lets me view and edit the `.forager/config.toml` for the currently selected
...
Repo tab, edit hooks, save, and verify `.forager/config.toml` is updated in
...
1. **Given** the user has a session selected on the home screen,
   **When** they open settings and navigate to the Repo tab, **Then**
   they see the repo-level hooks from `.forager/config.toml` (or empty
   fields if no config exists).
2. **Given** the user edits repo-level hooks and saves, **When** they
   inspect `.forager/config.toml` in the project directory, **Then** the
...
3. **Given** no session is selected (or the session has no project
   ...
   available.
4. **Given** the user adds hooks to a repo that had no `.forager/config.toml`,
   **When** they save, **Then** the file and `.forager/` directory are
...
- What happens when the global config defines hooks but the user has
  ...
  from `.forager/config.toml` require trust approval.
...
- Where do hooks execute? Hooks execute locally in the host project
  directory. The retired sandbox surface is limited to legacy metadata
  cleanup and does not create runnable hook environments.
- What happens if a legacy sandbox session is selected? Legacy sandbox
  records cannot be started as new runtime sessions. Hook execution for
  active sessions remains host-local.
...
- Q: Where do global/profile hooks execute? → A: All hooks run locally in the host project directory. No per-hook location override. Failure semantics (on_create=fatal, on_launch=non-fatal) apply uniformly regardless of config level.
...
- **FR-006**: The settings TUI MUST include a "Repo" tab that loads and
  saves `.forager/config.toml` from the currently selected session's project
  directory.
...
- **FR-008**: Saving repo settings MUST create the `.forager/` directory and
...
- **FR-011**: Hooks from all config levels (global, profile, repo) MUST
  execute locally in the host project directory. The retired sandbox
  surface MUST NOT introduce a separate runtime hook execution path.
...
- **RepoConfig**: The per-repository configuration loaded from
  `.forager/config.toml`. Currently contains hooks; the Repo tab exposes
...
- **SC-005**: Existing repo-level hook behavior (trust system and host
  execution semantics) remains unchanged.
